...
def consulta_inscritos(request):
    ...
    ids_comunidade_vinculados = Prontuario.objects.exclude(paciente_comunidade__isnull=True).values_list('paciente_comunidade_id', flat=True)
    ids_convenio_vinculados = Prontuario.objects.exclude(paciente_convenio__isnull=True).values_list('paciente_convenio_id', flat=True)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if tipo == 'comunidade':
                ...
                inscrito = get_object_or_404(Inscritocomunidade, idfichacomunidade=id_inscrito)
                ...
                inscrito = get_object_or_404(Inscritoconvenio, idfichaconvenio=id_inscrito)
